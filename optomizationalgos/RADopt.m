function [allsequences,bestsequence,cost] = RADopt(inputss, mpc,removedbuses,cutlines, origloss)
%Do RAD but sequence all subsequences optimally
...
for n = 1:numiter
    %generate partition
    %for optimal, the max partition size is 4
    ...
    for i = 1:20
        ...
        while true
           num = randi([partition(end)+2, partition(end) + min(ceil(numbuses/2), 4)]);
