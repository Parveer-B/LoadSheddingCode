function [allsequences,bestsequence,cost] = ngreedywrestorebsdmockcostold(n,mpc,removedbuses,cutlines, origloss)
    ...
    for j=1:size(removedbuses,1) %I could add another loop if we're iterating through more than one previous sequence
        if ~ismember(removedbuses(j, 1), cursequences.sequence)
            ...
            if isa(newsequences, 'double')
                newsequences = struct('sequence', toaddsequence, 'cost', cursequences.cost + cursequences.lossafteriter*(lines1 + lines2), 'mockcost', cursequences.mockcost +  iterrestored/totaltime + 0.0001/(lines1 + lines2), 'lossafteriter', lossafteriter, 'totalrestored', totalrestored, 'iterrestored', iterrestored, 'totaltime', totaltime);
            else
                newsequences = [newsequences; struct('sequence', toaddsequence, 'cost', cursequences.cost + cursequences.lossafteriter*(lines1 + lines2), 'mockcost', cursequences.mockcost +  iterrestored/totaltime + 0.0001/(lines1 + lines2), 'lossafteriter', lossafteriter, 'totalrestored', totalrestored, 'iterrestored', iterrestored, 'totaltime', totaltime)];
            end
        end
    end
